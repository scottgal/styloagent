...
- **Regenerating the figures.** Every figure in this manual is generated by
  `tests/Styloagent.UITests/ManualScreenshotTests.cs`. To refresh the PNGs under
  `docs/manual/images/`, run:
  ```bash
  dotnet test tests/Styloagent.UITests/Styloagent.UITests.csproj --filter "FullyQualifiedName~ManualScreenshotTests"
  ```
  (An earlier build break — `ArchitectureImpact.cs` using the `MermaidSharp` namespaces from Naiad
  without a direct project reference — has been resolved by referencing Naiad directly, so the suite
  builds and regenerates cleanly.)
- **The Document Library figure uses a deliberately flat doc tree.** Under headless render, a non-leaf
  `TreeViewItem` (a folder row) collapses its header to near-zero height, so a folder's first child
  paints over the folder's label. The *live* app renders folders correctly — this is a headless-only
  artifact of the Fluent theme's tree template (tracked as a filed issue in the cockpit's domain). To
